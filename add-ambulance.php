...
if(!isset($_SESSION['covid_app_user']) || $_SESSION['covid_app_user'] != 'admin'){
	$redirect = (isset($_GET['id'])?'redirect=add-ambulance.php?id='.$_GET['id']:'redirect=add-ambulance.php');
...
if(isset($_GET['id'])){
	...
	$query = "select * from ambulance_details where ambulance_id=?";
	...
	if($stmt = $con->prepare($query)){
		...
		if($stmt->execute()){
			...
			if(mysqli_num_rows($result) == 1){
				...
				$attr = json_decode($row['ambulance_attr'],true);
			}
		}
	}
}
...
<html lang="en">
  <head>
    ...
    <title>Add ambulance details</title>
  ...
  <body class="bg-light">
    ...
    <div class="main" style="margin: auto; width: 50%;">
    	<div class="main-inner" style="width: 80%; margin-top: 20px; padding: 10px; border: solid 1px lightgrey; border-radius: 10px; background-color: white; box-shadow: 2px 2px lightgrey;">
    		<h4 style="text-align: center">Ambulance details</h4>
    		...
    		<form id="hospital_form">
			  <div class="form-group">
			    ...
			    <input type="text" class="form-control" id="name" placeholder="Name of service provider" value="<?php if(isset($_GET['id'])) echo $row['ambulance_provider']; ?>"  required>
			  </div>
			  <div class="form-group">
			    <label for="phone">Phone numbers</label>
			    <input type="text" class="form-control" id="phone" placeholder="Phone numbers to book ambulance" value="<?php if(isset($_GET['id'])) echo $row['ambulance_phone']; ?>">
			  </div>
			  <div class="form-group">
			    <label for="num">Number of ambulances</label>
			    <input id="num" class="form-control" placeholder="Number of ambulances" type="number" value="<?php if(isset($_GET['id'])) echo $row['ambulance_qty']; ?>" />
			  </div>
			  <div class="form-group">
			    <label for="cost">Approx cost</label>
			    <input id="cost" type="text" class="form-control" placeholder="Approx cost of ambulance" value="<?php if(isset($_GET['id'])) echo $row['ambulance_cost']; ?>">
			  </div>
			  <div class="form-group">
			    <label for="types">Types of ambulances</label>
			    <input id="types" type="text" class="form-control" placeholder="Eg:- with ventilator, without ventilator" value="<?php if(isset($_GET['id'])) echo $row['ambulance_types']; ?>">
			  </div>
    ...
    $(document).ready(function() {
    	$("#hospital_form").submit(function(e){
    		...
    		if($("#submit_btn").html() == "Update"){
    			data1['update_ambulance'] = 1;
    			data1['amb_id'] = amb_id;
    		}
    		else{
    			data1['add_ambulance'] = 1;
    		}
